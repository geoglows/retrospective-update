...
def check_local_zarrs_match_s3(local_zarr_path: str, s3_zarr_path: str) -> None:
    ...
    if not (local_zarr['time'] == s3_zarr['time']).all():
        cl.error(f'Time arrays do not match in {local_zarr_path} and {s3_zarr_path}')
        raise EnvironmentError

    if not (local_zarr['river_id'] == s3_zarr['river_id']).all():
        cl.error(f'River id arrays do not match in {local_zarr_path} and {s3_zarr_path}')
        raise EnvironmentError

    if local_zarr['Q'].shape != s3_zarr['Q'].shape:
        cl.error(f'Q array shapes do not match between local {local_zarr_path} and s3 {s3_zarr_path}')
        raise EnvironmentError
...
def check_for_init_files() -> None:
    ...
    if result.returncode != 0:
        cl.error(f"Error running s5cmd copy to get init files")
        raise RuntimeError
