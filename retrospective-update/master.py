...
if __name__ == '__main__':
    try:
        ...
        f.setup_configs(configs_dir, GEOGLOWS_ODP_CONFIGS, ODP_CREDENTIALS_FILE, CL, s3, convert=True)

        CL.log_message('RUNNING', 'getting initial qinits')
        f.get_qinits_from_s3(s3, configs_dir, s3_qfinal_dir, outputs_dir, True)


        with Pool(f.processes(runoff_dir)) as p:
            CL.log_message('RUNNING', 'preparing inflows and namelists')
            f.inflow_and_namelist(runoff_dir, configs_dir, inflows_dir, namelists_dir, outputs_dir, HOME, p)
